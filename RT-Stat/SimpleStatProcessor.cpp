...
#include <stdio.h>
#include "SimpleStatProcessor.h"
#include "SimpleStatEditor.h"
//#include <chrono>


SimpleStatProcessor::SimpleStatProcessor()
: GenericProcessor("SimpleStat")
{
}
...
AudioProcessorEditor* SimpleStatProcessor::createEditor()
{
    editor = new SimpleStatEditor(this, true);
    return editor;
}


void SimpleStatProcessor::setParameter(int parameterIndex, float newValue)
{
    if (parameterIndex == 0)
...
void SimpleStatProcessor::process(AudioSampleBuffer& buffer, MidiBuffer& events)
{
...
SimpleStatProcessor::Neuron SimpleStatProcessor::make_Neuron(int64_t timestamp){
    Neuron neuron;
...
void SimpleStatProcessor::NeuronSUpdate(uint16_t sortedId, int64_t timestamp){
...
void SimpleStatProcessor::handleEvent(int eventType, MidiMessage& event, int sampleNum)
{
...
/* Should run:
 if (NeuronS[sortedId].countAP == SimpleStatProcessor::requiredSpikes)
 before this function */

void SimpleStatProcessor::createStandDevInfo(uint16_t sortedId){
...
/* Calculate ITI info */
double SimpleStatProcessor::get_CurrentITI(uint16_t sortedId){
    return NeuronS[sortedId].currentAP - NeuronS[sortedId].previousAP;
}


double SimpleStatProcessor::showAverageRate(uint16_t sortedId){
...
void SimpleStatProcessor::setOutputChan(int chan){
    outputChan = chan - 1;
}


void SimpleStatProcessor::updateSpikeRequirement(uint16_t userInput){
    requiredSpikes = userInput;
}


void SimpleStatProcessor::updateRequiredTime(int64 time){
    requiredTime += 1000*time - requestedTime;
    requestedTime =1000*time;
}


void SimpleStatProcessor::setDirection(int num){
...
void SimpleStatProcessor::setUserReady(int num){
    userReady = num;
...
void SimpleStatProcessor::setStandDevFactor(float num){
    standDevFactor = num;
}



void SimpleStatProcessor::setThresh(uint16_t sortedId){
    //sortedId -=1;
...
void SimpleStatProcessor::createHistoInfo(uint16_t sortedId){ //Take user visible sortedId or internal (1 vs 0 index)?
...
void SimpleStatProcessor::createHistoFactors(uint16_t sortedId){
...
bool SimpleStatProcessor::enable()
{
    SimpleStatEditor* editor = (SimpleStatEditor*) getEditor();
    editor->enable();
    return true;
...
unsigned int * SimpleStatProcessor::showHistoInfo(uint16_t sortedId){
    return & NeuronS[sortedId].histoInfo[0];
}


float * SimpleStatProcessor::showHistoFactors(uint16_t sortedId){
    return & NeuronS[sortedId].histoFactors[0];
}


bool SimpleStatProcessor::showSTDVStatus(uint16_t sortedId){
    return NeuronS[sortedId].shouldMakeSTDV;
}


double SimpleStatProcessor::showIncreaseThresh(uint16_t sortedId){
    return NeuronS[sortedId].increaseThresh;
}


double SimpleStatProcessor::showDecreaseThresh(uint16_t sortedId){
   return NeuronS[sortedId].decreaseThresh;
}

double SimpleStatProcessor::showMaxRate(uint16_t sortedId){
    return NeuronS[sortedId].maxRate;
}


double SimpleStatProcessor::showMinRate(uint16_t sortedId){
    return NeuronS[sortedId].minRate;
}


unsigned long SimpleStatProcessor::getNeuronSSize(){
    return NeuronS.size();
}


void SimpleStatProcessor::trainUpAll(){
...
void SimpleStatProcessor::trainDownAll(){
    for (unsigned int i = 0 ; i < NeuronS.size() ; i++)
...
int SimpleStatProcessor::showDirection (uint16_t sortedId){
    if(NeuronS.size() > 0)
...
double SimpleStatProcessor::showSTDV(uint16_t sortedId){
    return NeuronS[sortedId].standardDev;
}

void SimpleStatProcessor::resetAllThresh(){
    for(unsigned int i = 0 ; i < NeuronS.size() ; i++)
        setThresh(i);
}


double SimpleStatProcessor::binMe(unsigned int startBin, unsigned int endBin, double binSize, double zeroedRate){
...
long long SimpleStatProcessor::showSpikesToGo(uint16_t sortedId){
    return requiredSpikes - NeuronS[sortedId].countAP;
}
int64 SimpleStatProcessor::showTimeToGo(){
    return (requiredTime - Time::getCurrentTime().toMilliseconds())/1000; //return in seconds
}

unsigned long long SimpleStatProcessor::showMaximumCount(uint16_t sortedId){
    return NeuronS[sortedId].maximumCount;
}


void SimpleStatProcessor::trueShouldMakeHisto(uint16_t sortedId){
    NeuronS[sortedId].shouldMakeHistoInfo = true;
}

bool SimpleStatProcessor::showShouldMakeHistoInfoStatus(uint16_t sortedId){
    return NeuronS[sortedId].shouldMakeHistoInfo;
}

unsigned long long SimpleStatProcessor::showRewardCount(uint16_t sortedId){
